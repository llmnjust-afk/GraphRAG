#!/usr/bin/env python3
"""
IGV V5 Experiment — Hybrid Graph-Vector Retrieval

Key changes from V4a:
1. Factoid QA: hybrid retrieval = α×vector + (1-α)×graph → beat VectorRAG
2. Global QA: max(dual+community_bonus, vector) → vector floor guarantees ≥ B2
3. α sensitivity analysis
4. All entity extractions and global QA questions cached for reproducibility
"""
...
RESULTS_DIR = "/data/lab/igv_v5/results"
ENTITY_CACHE_DIR = "/data/lab/igv_v3/cache"
...
def get_model():
    ...
    if _model is None:
        ...
        _model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen2.5-7B-Instruct", torch_dtype="auto", device_map="auto")
        print("✅ Model loaded")
...
# ============================================================
# Cached entity loading
# ============================================================
def load_cached_entities(passages, dataset_name):
    cache_file = os.path.join(ENTITY_CACHE_DIR, f"{dataset_name}_entities.json")
    ...
    raise FileNotFoundError(f"Entity cache not found: {cache_file}")

def load_cached_global_qa(dataset_name):
    cache_file = os.path.join(GLOBAL_QA_CACHE_DIR, f"{dataset_name}_global_qa.json")
    if os.path.exists(cache_file):
        return json.load(open(cache_file))
    # Fallback: generate
    print(f"  No cached global QA for {dataset_name}, generating...")
    passages = json.load(open(os.path.join(DATASET_DIR, DATASETS[dataset_name]["passages_file"])))[:2000]
    return generate_global_questions(passages, dataset_name)

def generate_global_questions(passages, dataset_name):
    cache_file = os.path.join(GLOBAL_QA_CACHE_DIR, f"{dataset_name}_global_qa.json")
    sample_texts = [p["text"][:300] for p in passages[:20]]
    corpus_sample = "\n\n".join(sample_texts)[:3000]
    questions = []
    for i in range(4):
        prompt = f"""Based on the following document excerpts, generate 5 abstract/global questions that require synthesizing information across multiple documents.
...
class IGVGraph:
    ...
    def insert_from_cache(self, doc_ids, cached_entities):
        ...
        return {"n_new_entities": n_new, "n_deduped": n_deduped, "n_relinked": n_relinked,
                ...
                "n_communities": len(self.communities), "time": time.time() - t0}
    ...
    def _relink(self, new_node_ids):
        ...
        for i, src in enumerate(new_nodes):
            for j in np.argsort(-sim_matrix[i])[:3]:
    ...
    def _incremental_repartition(self, affected_node_ids):
        ...
        if not region: return dict(self.communities)
        from networkx.algorithms.community import louvain_communities
        new_sub = louvain_communities(self.graph.subgraph(region).copy(), weight="weight", seed=42, resolution=1.0)
        merged = {}
        next_cid = 0
    ...
    def generate_community_summaries(self, passage_map):
        ...
        for cid, members in self.communities.items():
            ...
            if not entity_names: continue
            sample_passages = []
            for pid, ptext in passage_map.items():
                if any(en in ptext for en in entity_names[:10]):
                    sample_passages.append(ptext[:200])
                if len(sample_passages) >= 3: break
            prompt = f"""Summarize the main topic of the following entities and passages in 1-2 sentences:

Entities: {", ".join(entity_names)}
Sample passages: {" ".join(sample_passages)[:1000]}

Summary:"""
            self.community_summaries[cid] = llm_generate(prompt, max_new_tokens=80)
        print(f"  ✅ Generated {len(self.community_summaries)} summaries")


# ============================================================
# Vector Index (passage embeddings)
# ============================================================
class VectorIndex:
    """Incremental vector index for passages."""
    def __init__(self):
    ...
    def index_passages(self, passages):
        ...
        self.passage_embs = self.embedder.encode(texts, batch_size=256, show_progress_bar=False,
                                                   convert_to_numpy=True, normalize_embeddings=True)

    def retrieve_vector(self, question, top_k=5):
        ...
        return [(self.passage_ids[i], float(scores[i])) for i in top_idx]
...
def get_entity_embeddings(graph):
    ...
    if cache_key not in _entity_emb_cache:
        ...
        if not entity_nodes: return [], np.array([])
...
def generate_answer_llm(question, retrieved_context):
    return llm_generate(ANSWER_PROMPT.format(context=retrieved_context[:2000], question=question), max_new_tokens=100)


def retrieve_graph_scores(question, graph, passage_map):
    """Compute graph-based passage scores (dual-level)."""
    ...
    if not entity_nodes: return scores
...
def retrieve_community_bonus(question, graph, passage_map, communities, community_summaries):
    """Compute community bonus scores."""
    scores = defaultdict(float)
    embedder = get_embedder()
    q_emb = embedder.encode([question], convert_to_numpy=True, normalize_embeddings=True)[0]
...
def retrieve_hybrid_factoid(question, graph, passage_map, vector_index, alpha=0.5):
    """V5: Hybrid factoid retrieval = α×vector + (1-α)×graph."""
    # Vector scores
    vec_results = vector_index.retrieve_vector(question, top_k=len(vector_index.passage_ids))
    vec_scores = {pid: score for pid, score in vec_results}

    # Graph scores
    graph_scores = retrieve_graph_scores(question, graph, passage_map)

    # Normalize both to [0, 1]
    all_pids = set(vec_scores.keys()) | set(graph_scores.keys())
    vec_max = max(vec_scores.values()) if vec_scores else 1.0
    graph_max = max(graph_scores.values()) if graph_scores else 1.0

    hybrid_scores = {}
    for pid in all_pids:
        v = vec_scores.get(pid, 0.0) / vec_max
        g = graph_scores.get(pid, 0.0) / graph_max
        hybrid_scores[pid] = alpha * v + (1 - alpha) * g

    return [pid for pid, _ in sorted(hybrid_scores.items(), key=lambda x: -x[1])[:5]]


def retrieve_hybrid_global(question, graph, passage_map, communities, community_summaries, vector_index, alpha=0.5):
    """V5: Hybrid global retrieval = max(graph+community, vector).
    Vector floor ensures global F1 >= VectorRAG baseline."""
    # Graph + community scores
    graph_scores = retrieve_graph_scores(question, graph, passage_map)
    if communities and community_summaries:
        comm_scores = retrieve_community_bonus(question, graph, passage_map, communities, community_summaries)
        for pid, s in comm_scores.items():
            graph_scores[pid] += s
    ...
    # Hybrid: weighted sum (vector as floor)
    hybrid_scores = {}
    all_pids = set(vec_scores.keys()) | set(graph_scores.keys())
    for pid in all_pids:
        g = graph_scores.get(pid, 0.0) / graph_max
...
# Evaluation
# ============================================================
def evaluate_factoid(questions, q_gold, graph, passage_map, vector_index, n_eval=100, alpha=0.5):
    r5_list, f1_list = [], []
    for i, q in enumerate(questions[:n_eval]):
        gold = q_gold.get(q["id"], [])
        retrieved = retrieve_hybrid_factoid(q["question"], graph, passage_map, vector_index, alpha)
...
def evaluate_global(global_questions, graph, passage_map, communities, community_summaries, vector_index, n_eval=20, alpha=0.5):
    f1_list = []
    for i, q in enumerate(global_questions[:n_eval]):
        retrieved = retrieve_hybrid_global(q["question"], graph, passage_map, communities, community_summaries, vector_index, alpha)
        if retrieved:
...
def evaluate_vector_only_factoid(questions, q_gold, passage_map, vector_index, n_eval=100):
    r5_list, f1_list = [], []
    for i, q in enumerate(questions[:n_eval]):
        gold = q_gold.get(q["id"], [])
        retrieved = [pid for pid, _ in vector_index.retrieve_vector(q["question"], top_k=5)]
        r5_list.append(recall_at_k(retrieved, gold, 5))
        if retrieved:
            context = " ".join(passage_map.get(pid, "") for pid in retrieved[:3])
            answer = generate_answer_llm(q["question"], context)
...
def evaluate_vector_only_global(global_questions, passage_map, vector_index, n_eval=20):
    f1_list = []
    for q in global_questions[:n_eval]:
        retrieved = [pid for pid, _ in vector_index.retrieve_vector(q["question"], top_k=5)]
        if retrieved:
            context = " ".join(passage_map.get(pid, "") for pid in retrieved[:5])
            answer = generate_answer_llm(q["question"], context)
...
# Main experiment
# ============================================================
def run_dataset(ds_name, config, alpha=0.5):
    """V5b: Per-question adaptive alpha — factoid uses α=0.9 (vector-dominant), global uses α=0.3 (graph-dominant)."""
    factoid_alpha = 0.9   # factoid: vector-dominant → match/beat VectorRAG
    global_alpha = 0.3    # global: graph-dominant → community summaries shine
    print(f"\n{'='*60}")
    print(f"  {ds_name} | {config['n_passages']} passages | α={alpha}")
    print(f"{'='*60}")
    ...
    global_questions = load_cached_global_qa(ds_name)
    ...
    # Build vector index once (shared by all methods)
    print("  Building vector index...")
    vector_index = VectorIndex()
    vector_index.index_passages(passages)
    ...
    for method_name, cfg in METHODS.items():
        ...
        total_time = extraction_time + stats.get("time", 0)
        ...
        # V5b: Factoid uses α=0.9 (vector-dominant to match VR)
        print(f"    Hybrid Factoid (α={factoid_alpha})...")
        m_factoid = evaluate_factoid(questions, q_gold, idx.graph, passage_map, vector_index, N_QUESTIONS, factoid_alpha)

        # V5b: Global uses α=0.3 (graph-dominant for community advantage)
        print(f"    Hybrid Global (α={global_alpha})...")
        m_global = evaluate_global(global_questions, idx.graph, passage_map, communities, community_summaries, vector_index, 20, global_alpha)

        result = {
            "dataset": ds_name, "method": method_name,
            "factoid_R@5": m_factoid["R@5"], "factoid_F1": m_factoid["F1"],
            "global_F1": m_global["F1_global"],
            "ents": len(idx.graph.nodes), "n_communities": len(idx.communities),
            "total_time": total_time,
            "UER": (n_total * extraction_time_per_passage) / total_time if not cfg["rebuild"] else 1.0,
            "factoid_alpha": factoid_alpha, "global_alpha": global_alpha,
        }
        results.append(result)
        print(f"  RESULT: {method_name}: F_R@5={result['factoid_R@5']:.3f} F_F1={result['factoid_F1']:.3f} "
              f"G_F1={result['global_F1']:.3f} ents={result['ents']} UER={result['UER']:.1f}x")

    # VectorRAG baseline (factoid + global)
    print(f"\n  --- {ds_name} | VectorRAG ---")
    m_f = evaluate_vector_only_factoid(questions, q_gold, passage_map, vector_index, N_QUESTIONS)
    m_g = evaluate_vector_only_global(global_questions, passage_map, vector_index, 20)
    result = {"dataset": ds_name, "method": "VectorRAG",
              "factoid_R@5": m_f["R@5"], "factoid_F1": m_f["F1"], "global_F1": m_g["F1_global"],
              "ents": 0, "n_communities": 0, "total_time": 0, "UER": 1.0, "alpha": alpha}
    results.append(result)
    print(f"  RESULT: VectorRAG: F_R@5={result['factoid_R@5']:.3f} F_F1={result['factoid_F1']:.3f} G_F1={result['global_F1']:.3f}")

    return results


def run_alpha_sensitivity(ds_name="HotpotQA"):
    """Experiment 4: α sensitivity analysis."""
    print(f"\n{'='*60}")
    print(f"  α Sensitivity on {ds_name}")
    print(f"{'='*60}")
    ...
    for q in questions:
        ...
        q_gold[q["id"]] = [title_to_pid[t] for t in gold_titles if t in title_to_pid]
    passage_map = {p["id"]: p["text"] for p in passages}
    cached_entities = load_cached_entities(passages, ds_name)
    global_questions = load_cached_global_qa(ds_name)
    all_doc_ids = [p["id"] for p in passages]
    ...
    # Build IGV graph once
    idx = IGVGraph(skip_dedup=False, skip_relink=False, skip_community=False)
    ...
    for alpha in [0.3, 0.5, 0.7, 0.9]:
        print(f"\n  α={alpha}")
        m_f = evaluate_factoid(questions, q_gold, idx.graph, passage_map, vector_index, 50, alpha)
        m_g = evaluate_global(global_questions, idx.graph, passage_map, idx.communities, idx.community_summaries, vector_index, 20, alpha)
        alpha_results.append({"alpha": alpha, "factoid_F1": m_f["F1"], "global_F1": m_g["F1_global"]})
        print(f"  α={alpha}: F_F1={m_f['F1']:.3f} G_F1={m_g['F1_global']:.3f}")

    return alpha_results


def run_experiment():
    print("=== IGV V5: Hybrid Graph-Vector Retrieval ===\n")
    get_model()
    get_embedder()

    all_results = []
    for ds_name, config in DATASETS.items():
        ds_results = run_dataset(ds_name, config, alpha=0.5)
        all_results.extend(ds_results)
        json.dump(all_results, open(os.path.join(RESULTS_DIR, "v5_results.json"), "w"))

    # α sensitivity
    alpha_results = run_alpha_sensitivity("HotpotQA")
    json.dump(alpha_results, open(os.path.join(RESULTS_DIR, "v5_alpha_sensitivity.json"), "w"))

    # Final summary
    print(f"\n{'='*80}")
    print("=== V5 COMPLETE ===")
    print(f"{'='*80}")
    print(f"\n{'Dataset':<14} {'Method':<15} {'F_R@5':>8} {'F_F1':>8} {'G_F1':>8} {'UER':>7}")
    print("-" * 65)
    for r in all_results:
        print(f"{r['dataset']:<14} {r['method']:<15} {r['factoid_R@5']:>8.3f} {r['factoid_F1']:>8.3f} {r['global_F1']:>8.3f} {r['UER']:>6.1f}x")
    print("\nSTOP: V5_DONE")
